...
Сама структура проекта выглядит следующим образом (обновлено):
...
8. Далее проверять работу на `localhost:5000` или по ip устройства (например, `<ваш_ip:5000>`). Логин и пароль админа для проверки прав: `admin / admin`. Для проверки обычных прав можно зарегистрировать своего пользователя.

> Чтобы выйти из виртуального окружения, наберите команду `deactivate`.

### Пароль на почтовом сервере больше неактуален. Для проверки работоспособности используйте свой пароль приложений (при наличии яндекса процесс проще. Если используется другая служба, необходимо будет менять конфиг)

> #

## [18-05-2024] Список изменений (v 0.8.4):
* Футер: добавлена ссылка на страницу доната 
* Вернулась кнопка "Очистить галерею" в боковом меню для admin пользователя
* Исправлен слой изображении. Теперь поведение при наведении более корректно
* Мелкая оптимизация кода: python-файлы разделены для более удобного управления проектом

## [16-05-2024] Список изменений (v 0.8.0):
<ul>
      <li> У сайта новое название <b>GuardiArt</b>. Но сам проект всё ещё имеет название "Простая Онлайн Галерея".
      <li> Дизайн: 
            <ul>
            <li> изменены расположения кнопок
            <li> изменен дизайн кнопки добавления изображения (плюсик в облачке)
            <li> изменен дизайн кнопки "очистить галерею". Углы кнопки более округлые
            <li> кнопка удаления только своих изображений из окна профиля была перенесена в пункт меню профиля
            <li> мелкие доработки бокового меню
            <li> в модальное окно регистрации добавлено поле почтового адреса
            </ul>
      <li> Регистрация на сайте теперь также требует электронную почту пользователя, на которую присылается ссылка активации аккаунта
      <li> Изменена таблица 'users' в бд
      <li> Добавлена реклама сайта "партнёра" в качестве примера монетизации
      <li> Футер: Добавлена ссылка на телеграм автора
      <li> Мелкие изменения в html коде и его оптимизация
</ul>

## [04-05-2024] Список изменений (v 0.7.8):
<ul>
      <li> Добавлена кнопка загрузки изображения
      <li> Добавлен скрипт, обрабатывающий клик ПКМ по изображению. Скачивать можно только авторизованным пользователем
      <li> Исправлена высота и позиция overlay
      <li> Мелкие правки в коде проекта и оптимизация index.html
</ul>
...
<ul>
      <li> Дизайн:
      <ul>
            <li> изменен стиль кнопок в боковом меню и их положение
            <li> превью картинок теперь не на весь размер. Изменены максимальная высота и ширина
      </ul>
</ul>
...
<ul>
      <li> Дизайн:
      <ul>
            <li> переработан внешний вид
            <li> появилось боковое меню. Отображается, если ширина экрана достигает 1030px и меньше
            <li> отображение имени автора и названия изображения, которое автор мог написать при добавлении изображения
            <li> изменен внешний вид сетки изображений
            <li> изменен внешний вид модального окна изображения
            <li> цвет акцента кнопок авторизации изменился на зелёный
      </ul>
<li> Анимации:
      <ul>
            <li> при уменьшеной ширине экрана все основные элементы (добавление, удаление, очистка, поиск, меню профиля) переносятся в сайдбар меню
            <li> изменение поведения шапки. При скролле страницы она опускается до начала надписи "Простая Онлайн Галерея". Так основные элементы будут отображаться и при этом экономить основной контент
      </ul>
<li> Появился профиль для каждого пользователя, с возможностью просмотра личных изображений
<li> Появилось затеменение изображений при наведении
<li> Добавлена возможность просмотреть только изображения профиля
<li> Добавлена кнопка удаления всех пользовательских изображений. Это не то же самое, что очистка всей галереи (доступна только для админа)
<li> Поиск: теперь можно искать изображения и по названию, и по имени автора
<li> Мелкие изменения в футере: появилась ссылка на github проект
<li> Удалена база данных с изображениями. Теперь в базе account.db есть две необходимые таблицы: users и images
</ul>

...
<details> 
  <summary>Помощь по активации виртуального окружения (для Windows)</summary>
     Активировать окружение через команду venv\Scripts\Activate.ps1 
</details>
<details> 
  <summary>Помощь по активации виртуального окружения (для Linux)</summary>
    Активировать окружение через команду venv\bin\activate
</details>

5. Установить библиотеки и зависимости через `pip install -r requirements.txt`
6. Выполнить команду `py app.py` (или `flask run --host=0.0.0.0`   Это позволит серверу принимать запросы не только с локальной машины, но и с других устройств локальной сети.)
7. Далее проверять работу на `localhost:5000` или по ip устройства (например, `<ваш_ip:5000>`). Логин и пароль админа и модератора для проверки прав: `admin / admin`, `user / user`. Для проверки обычных прав можно зарегистрировать своего пользователя.
